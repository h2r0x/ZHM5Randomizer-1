...
uint64_t __fastcall SceneLoadObserver::detour(
    void* this_, SSceneInitParameters* scene_init_params) {
  for (const auto& callback : load_scene_callbacks) callback(scene_init_params);

  return o_load_scene(this_, scene_init_params);
}

void SceneLoadObserver::registerSceneLoadCallback(
    const tLoadSceneCallback& callback) {
