...
// complex hooks or removable hooks are ever required.
int MemoryUtils::DetourCall(void* hook_call_addr, const void* hook_function) {
  if (trampoline_memory_base == nullptr)
    trampoline_memory_base =
        VirtualAlloc((LPVOID)0x17F000000, 0x200, MEM_COMMIT | MEM_RESERVE,
  ...
  // build hook jmp
  int jmp_operand =
      (int)((long long)trampoline_addr - ((long long)hook_call_addr + 5));
  ...
  *(int*)(hook_bytes + 1) = jmp_operand;
  ...
  unsigned char trampoline_bytes[TRAMPONLINE_SIZE]{
      ...
      0x00, 0x00, 0x00, 0x00, 0x00,  // mov rax, XXXXXXXXXXXXXXXX
      0xFF, 0xD0,                    // call rax
      0xE9, 0x00, 0x00, 0x00, 0x00   // jmp rip+XXXXXXXX
  };

  *(long long*)(trampoline_bytes + 2) = (long long)hook_function;
  int return_jmp_operand =
      ((long long)hook_call_addr + 5) - ((long long)trampoline_addr + 17);
  *(int*)(trampoline_bytes + 13) = return_jmp_operand;
  memcpy_s(trampoline_addr, sizeof(trampoline_bytes), trampoline_bytes,
           sizeof(trampoline_bytes));

  return 0;
}

void MemoryUtils::DetourVFTCall(void** vft_entry_addr, void* hook_function,
                                void** original_fn_ptr) {
  *original_fn_ptr = *vft_entry_addr;

  DWORD oldProtection;
  VirtualProtect(vft_entry_addr, sizeof(void*), PAGE_EXECUTE_READWRITE,
                 &oldProtection);
